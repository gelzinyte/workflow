...
def calc(inputs, calc_property_prefix, ref_property_prefix,
    ...
    for prop in all_diffs:
        ...
        for cat in all_diffs[prop]:
            ...
            all_errors[prop][cat] = {'RMSE': RMSE / kcal * mol, 'MAE': MAE / kcal * mol, 'count' : num}
...
def select_units(prop, plt_type):
    ...
    units_dict = {
        "energy": {"parity": ("kcal/mol", 1.0), "error": ("kcal/mol", 1.0)},
        "energy/atom": {"parity": ("kcal/mol/at", 1.0), "error": ("kcal/mol/at", 1.0)},
        "forces": {"parity": ("kcal/mol/Å", 1.0), "error": ("kcal/mol/Å", 1.0)},
        # "virial": {"parity": ("eV", 1.0), "error": ("meV", 1.0e3)},
        # "virial/atom": {"parity": ("eV/at", 1.0), "error": ("meV/at", 1.0e3)}
    }
